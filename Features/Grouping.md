...
The mapping runs at the first-level, that is, for each group. The maper expression `. %>>% list.mapv(Name)` means that each people in the group maps to the name.
